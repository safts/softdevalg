...
int AddCentroid(int centrIndex) {
	...
	//printf("%d, %d\n", centrIndex, dataAct.centroidsIndex);
...
double Silhouette(FILE* output){
	...
	#pragma omp parallel for reduction(+:sTotal)
	for(i=0; i<GetDataSize()-dataAct.k; i++){
	...
	//for(i=GetDataSize()-dataAct.k; i < GetDataSize(); i++){
		//sAvg[i - GetDataSize()-dataAct.k ] += 1;
	//}
...
void parseData(FILE* file, double r) {
	dataAct.data = NULL;
	dataAct.clustIndexes = NULL;
	dataAct.centroidNum = 0;
	dataAct.k = 0;
	data.readData(&dataAct.data, file,r);
	dataAct.centroidsIndex = dataAct.dataSize;
...
void PrintConform(FILE* output){
	...
	for(i=0; i<dataAct.k; i++){
		...
		for(j=0; j<size; j++){
			fprintf(output, "%d ",*(int*)(clustArr[j])->content );
		}
		GetIthCentroid(i,&centr);
		fprintf(output, "%d\n",*(int*)centr.content );
		free(clustArr);
	}
}
