...
/*{{{  data*/
void ReadDataDistanceMatrix(FILE* inputFile, int k) {
	List tempList;
	...
	dm->data= malloc(num* sizeof(value));

	char* tempVal = GetFirst(tempList);
	for (i = 0; i < num; i++) {
		dm->data[i].name = malloc(LABELSIZE * sizeof(char));
		memset(dm->data[i].name,'\0',LABELSIZE);
		if(tempVal ==NULL)
			continue;
		strcpy(dm->data[i].name, tempVal);
	...
	while (getline(&lineBuff, &lineSize, inputFile) != -1) {
		...
		dm->data[line].content=malloc(sizeof(int));
		memcpy(dm->data[line].content,&line,sizeof(int));
		...
		for (i = 0; i < num; i++) {
			...
			//printf("%d ", (int)dm->distances[line][i]);
			token = strtok(NULL, delims);
		}
		line++;
	}
...
void printEverything(){
	...
	for(i=0;i<dm->dataNum;i++){
		if(!(i%100==0))
			continue;
		printf("ID: %s, Values: ",dm->data[i].name);
		for(j=0;j<dm->dataNum;j++){
			int * index=dm->data[i].content;
			printf("%d ", (int) dm->dataDist[ *index][j]);
		}
		printf("\n");
	}
